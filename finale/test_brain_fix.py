...
# ==========================================
# FIX: Full list of candidate ports (added ttyACM1 - boards with native USB
# CDC, like many ESP32-S2/S3/C3 dev boards, commonly enumerate here if any
# other serial device is already present), and the exceptions are now
# surfaced instead of being silently swallowed.
ESP32_CANDIDATE_PORTS = ['/dev/ttyUSB0', '/dev/ttyUSB1', '/dev/ttyACM0', '/dev/ttyACM1', '/dev/serial0']


def connect_to_esp32():
    print("\n🔌 Searching for ESP32 via USB...")
    for port in ESP32_CANDIDATE_PORTS:
        try:
            s = serial.Serial(port, 115200, timeout=1)
            # FIX: Opening a serial port toggles DTR/RTS on most USB-serial
            # adapters, which reboots the ESP32. Give it time to finish
            # booting (WiFi AP + sensor init) before we trust the link, and
            # drop any boot-time garbage sitting in the input buffer.
            time.sleep(2.0)
            s.reset_input_buffer()
            print(f"✅ Successfully connected to ESP32 on {port}")
            return s
        except Exception as e:
            # FIX: Show the real reason this port failed (permission denied,
            # no such file, device busy, etc.) instead of hiding it.
            print(f"  ⚠️  {port}: {e}")
            continue
    print("❌ Failed to find ESP32 on USB. Manual mode will simulate commands.")
    print(f"   Checked ports: {', '.join(ESP32_CANDIDATE_PORTS)}")
    print("   If every port above shows 'Permission denied', run with sudo")
...
def reconnect_esp32():
    """FIX: Attempt to re-open the USB link if it drops mid-session - e.g. a
    brownout reset on the ESP32 caused by servo current draw, or a transient
    USB disconnect. Rate-limited so a missing cable doesn't spin in a tight loop."""
    global esp32_serial, last_reconnect_attempt
    now = time.time()
    ...
    while True:
        if esp32_serial and esp32_serial.is_open:
            try:
                ...
                # FIX: Surface read errors and try to recover instead of
                # spinning silently forever on a dead connection.
                consecutive_errors += 1
                if consecutive_errors == 1:
                    print(f"\n⚠️  ESP32 read error: {e}")
                if consecutive_errors >= 5:
                    print("🔄 Connection looks dead - attempting to reconnect...")
                    reconnect_esp32()
                    consecutive_errors = 0
                time.sleep(0.1)
        else:
            # FIX: No connection (or it was lost) - keep retrying instead of
            # spinning forever with a connection that will never come back.
            time.sleep(1.0)
            reconnect_esp32()
...
def send_to_esp32(command):
    if not (esp32_serial and esp32_serial.is_open):
        print(f"⚠️  Cannot send '{command}' - ESP32 not connected. Trying to reconnect...")
        reconnect_esp32()
        return
    try:
        esp32_serial.write((command + "\n").encode('utf-8'))
        esp32_serial.flush()
        # FIX: Added this print so you know exactly when Python sends a command
        print(f"\n📡 [PI SENT]: {command}")
    except Exception as e:
