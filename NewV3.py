...
def _reset_runtime_state():
    """
    Clear all cached audio/beat/genre state between audio loop restarts.
    Prevents stale BPM history, partial captures, and old YAMNet scores
    from triggering wrong dance moves or ghost voice commands on restart.
    """
    global _capture_buf, _capturing, _capture_start

    with state.lock:
        # Beat tracker — full reset, not soft_clear
        state.beat_tracker.bpm_history.clear()
        state.beat_tracker.beat_times.clear()
        state.beat_tracker.smoothed_bpm     = 0.0
        state.beat_tracker.beat_interval    = 0.5
        state.beat_tracker.next_beat_time   = 0.0
        state.beat_tracker.beat_confidence  = 0.0
        state.beat_tracker._phase_error_avg = 0.0

        # YAMNet smoothed scores — reset to uniform (no genre bias)
        state.genre_scores = np.zeros(521)
        state.genre        = "Listening..."

        # Movement state
        state.bpm          = 0.0
        state.beat_hit     = False
        state.music_speed  = "IDLE"
        state.last_dance_move         = ""
        # Push last_dance_command_time 3s into the future so we don't
        # fire immediately on the first beat after restart
        state.last_dance_command_time = time.time() + 3.0

        # VAD
        state.vad.reset()
        state.vad.noise_floor = 0.02   # back to safe default
        state.voice_active    = False
        state.voice_override_until = 0.0
    ...
    # Pre-buffer — discard stale audio from before restart
    with _pre_buf_lock:
        _pre_buf.clear()

    # Ring buffers — zero out so YAMNet doesn't process stale audio
    _audio_ring._buf[:] = 0.0
    _audio_ring._head   = 0
    _yamnet_win._buf[:] = 0.0
    _yamnet_win._head   = 0

    print("audio_listener: state cache cleared.")
...
def _run_audio_loop():
    ...
    # Clear all stale state before opening the mic.
    # Prevents ghost beats/moves from previous run bleeding into this loop.
    ...
    # Second reset after device probe (probe itself can take >100ms,
    # during which time.time() advances — re-stamp last_dance_command_time)
    with state.lock:
        state.last_dance_command_time = time.time() + 3.0
